---
name: yt-chat
description: Chat with transcribed YouTube video content — ask questions about a video's transcript and get grounded answers. Supports speaker-aware queries when diarization is enabled. Use when asked to chat with a video, ask questions about a YouTube video's content, query transcript content, or discuss what was said in a video. Requires the youtube-transcriber stack on localhost:8000 and a previously transcribed video (use yt-transcribe skill first if needed).
---
...
1. The youtube-transcriber stack must be running on `localhost:8000`
...
When diarization is enabled, the transcript includes speaker labels:

```bash
python3 scripts/chat.py --video-id <uuid> -q "What did Speaker 1 say about pricing?"
python3 scripts/chat.py --video-id <uuid> -q "How many speakers are in this video?"
python3 scripts/chat.py --video-id <uuid> -q "Summarize each speaker's main arguments"
```

The context sent to the LLM includes speaker-labeled, timestamped segments:
```
...
Uses semantic search to find relevant transcript chunks across all videos.
...
- `diarization_enabled` — whether speaker info is available
- `segments[].speaker` — speaker label per segment
...
- Speaker-aware context enables queries about individual speakers
